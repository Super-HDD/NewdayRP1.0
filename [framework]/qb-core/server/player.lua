...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.items = GetResourceState("qb-inventory") ~= "missing"
            and exports["qb-inventory"]:LoadInventory(PlayerData.source, PlayerData.citizenid)
        or {}
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    self.Functions.AddLevel = function(level, amount)
        ...
        if self.PlayerData.metadata['levels'][level] and amount > 0 then
            ...
            self.Functions.UpdatePlayerData()
            TriggerEvent('qb-log:server:CreateLog', 'levels', 'AddXp', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' [CitizenID: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..']** got '..level..' levels '..amount..' [current level: '..self.PlayerData.metadata['levels'][level])
            return true
        end
        return false
    end
    ...
    self.Functions.RemoveLevel = function(level, amount)
        ...
        if self.PlayerData.metadata['levels'][level] and amount > 0 then
            ...
            self.Functions.UpdateLevelData(level)
            self.Functions.UpdatePlayerData()
            TriggerEvent('qb-log:server:CreateLog', 'levels', 'RemoveXp', 'lightgreen', '**'..GetPlayerName(self.PlayerData.source) .. ' [CitizenID: '..self.PlayerData.citizenid..' | id: '..self.PlayerData.source..']** lost '..level..' levels '..amount..' current level: '..self.PlayerData.metadata['levels'][level])
            return true
        end
        return false
    end
    ...
    function self.Functions.RemoveMoney(moneytype, amount, reason)
        ...
        if moneytype == 'cash' then
            if self.PlayerData.money[moneytype] == self.PlayerData.money['cash'] then
                exports['ndrp-money']:RemoveMoney(PlayerData, amount, reason)
            end
        --[[ elseif moneytype == 'blackmoney' then
            if self.PlayerData.money[moneytype] == self.PlayerData.money['blackmoney'] then
                exports['ndrp-money']:RemoveMoney(PlayerData, amount, reason, 'blackmoney')
            end
        elseif moneytype == 'printedbills' then
            if self.PlayerData.money[moneytype] == self.PlayerData.money['printedbills'] then
                exports['ndrp-money']:RemoveMoney(PlayerData, amount, reason, 'printedbills')
            end ]]
        else
            self.PlayerData.money[moneytype] = self.PlayerData.money[moneytype] - amount
        end
    ...
    function self.Functions.GetMoney(moneytype)
        ...
        exports['ndrp-money']:UpdateCash(src)
