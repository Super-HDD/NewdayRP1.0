...
-- sell small fish
RegisterServerEvent('qb-fishvendor:server:sellsmallfish')
AddEventHandler('qb-fishvendor:server:sellsmallfish', function()
    local src = source
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        if hassmallfish == true then
            ...
            QBCore.Functions.Notify(source, Lang:t('success.small_fish_sold')..price, 'success')
            hassmallfish = false
        else
            QBCore.Functions.Notify(source, Lang:t('error.no_small_fish'), 'error')
        end
    end
end)

-- sell medium fish
RegisterServerEvent('qb-fishvendor:server:sellmediumfish')
AddEventHandler('qb-fishvendor:server:sellmediumfish', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local price = 0
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        if hasmediumfish == true then
            Player.Functions.AddMoney("cash", price, "fish-sold")
            QBCore.Functions.Notify(source, Lang:t('success.medium_fish_sold')..price, 'success')
            hasmediumfish = false
        else
            QBCore.Functions.Notify(source, Lang:t('error.no_medium_fish'), 'error')
        end
    end
end)

-- sell large fish
RegisterServerEvent('qb-fishvendor:server:selllargefish')
AddEventHandler('qb-fishvendor:server:selllargefish', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local price = 0
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        if haslargefish == true then
            Player.Functions.AddMoney("cash", price, "fish-sold")
            QBCore.Functions.Notify(source, Lang:t('success.large_fish_sold')..price, 'success')
            haslargefish = false
        else
            QBCore.Functions.Notify(source, Lang:t('error.no_large_fish'), 'error')
        end
    end
end)
