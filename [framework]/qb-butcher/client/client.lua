...
-- butcher menu
RegisterNetEvent('qb-butcher:client:menu', function(butchername)
    exports['qb-menu']:openMenu({
        ...
        },
        {
            ...
            icon = "fas fa-paw",
            params = {
                event = 'qb-butcher:client:sellanimal',
                isServer = false,
                args = {}
            }
        },
        {
            ...
            params = {
                event = 'qb-butcher:client:OpenButcherShop',
                isServer = false,
                args = {}
            }
        },
...
RegisterNetEvent('qb-butcher:client:sellanimal')
AddEventHandler('qb-butcher:client:sellanimal', function()
    ...
    if holding ~= false then
        for i = 1, #Config.Animal do
            if model == Config.Animal[i].model then
                ...
                QBCore.Functions.Progressbar('sell-carcass',  Lang:t('progressbar.selling')..name..'..', Config.SellTime, false, true, {
                    ...
                    if deleted then
                        if quality == false then
                            TriggerServerEvent('qb-butcher:server:reward', rewardmoney, rewarditem, 'poor') -- poor quality reward
                        end
                        if quality == 0 then
                            TriggerServerEvent('qb-butcher:server:reward', rewardmoney, rewarditem, 'poor') -- poor quality reward
                        end
                        if quality == 1 then
                            TriggerServerEvent('qb-butcher:server:reward', rewardmoney, rewarditem, 'good') -- good quality reward
                        end
                        if quality == 2 then
                            TriggerServerEvent('qb-butcher:server:reward', rewardmoney, rewarditem, 'perfect') -- perfect quality reward
                        end
                        if quality == -1 then
                            TriggerServerEvent('qb-butcher:server:reward', rewardmoney, rewarditem, 'perfect') -- perfect quality reward
                        end
        ...
        QBCore.Functions.Notify(Lang:t('error.dont_have_animal'), 'error')
    end
end)
...
RegisterNetEvent('qb-butcher:client:OpenButcherShop')
AddEventHandler('qb-butcher:client:OpenButcherShop', function()
    local ShopItems = {}
