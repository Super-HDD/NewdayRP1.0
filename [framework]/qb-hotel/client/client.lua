...
-- hotel menu
RegisterNetEvent('qb-hotel:client:menu', function(hotelname, hotellocation)
    ...
    exports['qb-menu']:openMenu({
        ...
        },
        {
            ...
            icon = "fas fa-bed",
            params = {
                event = 'qb-hotel:server:RentRoom',
                isServer = true,
                args = { location = hotellocation }
            }
        },
...
RegisterNetEvent('qb-hotel:client:gotoRoom', function(location)
...
Citizen.CreateThread(function()
    for hotelexit, v in pairs(Config.HotelRoom) do
        exports['qb-core']:createPrompt(v.prompt, v.coords, QBCore.Shared.Keybinds['J'], Lang:t('menu.room_menu'), {
            type = 'client',
            event = 'qb-hotel:client:roommenu',
            args = { v.location },
        })
    end
end)
...
-- room menu
RegisterNetEvent('qb-hotel:client:roommenu', function()
    QBCore.Functions.TriggerCallback('qb-hotel:server:GetActiveRoom', function(result)
        local activeRoom = {
            {
        ...
        activeRoom[#activeRoom+1] = {
            ...
            params = {
                event = "qb-hotel:client:addcredit",
                isServer = false,
        ...
        activeRoom[#activeRoom+1] = {
            ...
            icon = "fas fa-box",
            params = {
                event = "qb-hotel:client:roomlocker",
                isServer = false,
        ...
        activeRoom[#activeRoom+1] = {
            ...
            params = {
                event = 'qb-hotel:client:leaveroom',
                isServer = false,
        ...
        exports['qb-menu']:openMenu(activeRoom)
    end)
end)

--------------------------------------------------------------------------------------------------

RegisterNetEvent('qb-hotel:client:addcredit', function(data)
    ...
    if dialog ~= nil then
        for k,v in pairs(dialog) do
            ...
            TriggerServerEvent('qb-hotel:server:addcredit', tonumber(newcredit), tonumber(dialog.addcredit), data.room)
        end
    end
end)
...
-- leave room
RegisterNetEvent('qb-hotel:client:leaveroom')
AddEventHandler('qb-hotel:client:leaveroom', function(data)
    if Config.Debug == true then
        print(data.exitroom)
    end
    ...
    if roomlocation == 'valentine' then
        DoScreenFadeOut(500)
        Wait(1000)
        TriggerServerEvent('qb-hotel:server:setdefaultbucket')
...
-- room storage locker
RegisterNetEvent('qb-hotel:client:roomlocker', function(data)
...
RegisterNetEvent('qb-hotel:client:minibar')
AddEventHandler('qb-hotel:client:minibar', function(data)
    local ShopItems = {}
