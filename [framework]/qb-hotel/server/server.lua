...
-- rent room
RegisterNetEvent('qb-hotel:server:RentRoom', function(data)
    local src = source
    ...
    if result == 0 then
        local roomid = QBCore.Player.CreateRoomId()
        ...
        if cashBalance >= credit then
            ...
            QBCore.Functions.Notify(src, Lang:t('success.you_rented_room')..roomid, 'success')
        else
            QBCore.Functions.Notify(src, Lang:t('error.not_enought_cash_to_rent_room'), 'error')
        end
    else
        QBCore.Functions.Notify(src, Lang:t('error.you_already_have_room_here'), 'error')
    end
end)

-- room check-in
RegisterNetEvent('qb-hotel:server:CheckIn', function(data)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local citizenid = Player.PlayerData.citizenid
    local location = data.location
    ...
    if result ~= 0 then
        ...
        TriggerClientEvent('qb-hotel:client:gotoRoom', src, location)
    else
        QBCore.Functions.Notify(src, Lang:t('error.you_dont_have_room_here'), 'error')
    end
end)
...
QBCore.Functions.CreateCallback('qb-hotel:server:GetActiveRoom', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-hotel:server:addcredit', function(newcredit, removemoney, roomid)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local cashBalance = Player.PlayerData.money["cash"]
    if cashBalance >= removemoney then
        ...
        QBCore.Functions.Notify(src, Lang:t('success.room_credit_added_for')..roomid, 'success')
        Wait(5000)
        QBCore.Functions.Notify(src,  Lang:t('primary.your_credit_is_now')..newcredit, 'primary')
    else
...
RegisterNetEvent('qb-hotel:server:setdefaultbucket', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
-- create unique room id / -- QBCore.Player.CreateRoomId()
function QBCore.Player.CreateRoomId()
    local UniqueFound = false
    local RoomId = nil
    while not UniqueFound do
        RoomId = (QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomInt(2))
    ...
    if result then
        for i = 1, #result do
            ...
            if row.credit >= Config.RentPerCycle then
                ...
                TriggerEvent('qb-log:server:CreateLog', 'hotel', 'Hotel Room Lost', 'red', row.fullname..' room '..row.roomid..' in '..row.location..' has been deleted')
            end
        end
    end
