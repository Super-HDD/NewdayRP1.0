...
RegisterNetEvent('qb-trapperplus:server:storepelt')
AddEventHandler('qb-trapperplus:server:storepelt', function(rewarditem1, rewarditem2)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    Player.Functions.AddItem(rewarditem1, 1)
	Player.Functions.AddItem(rewarditem2, 1)
    TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[rewarditem1], "add")
	TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items[rewarditem2], "add")
end)

RegisterServerEvent('qb-trapperplus:server:sellpelts')
AddEventHandler('qb-trapperplus:server:sellpelts', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local price = 0
    ...
    if Player.PlayerData.items ~= nil and next(Player.PlayerData.items) ~= nil then 
        ...
        if haspelts == true then
            ...
            QBCore.Functions.Notify(source, Lang:t('success.you_have_sold_all_your_pelts_for')..price, 'success')
            haspelts = false
        else
            QBCore.Functions.Notify(source, Lang:t('error.you_dont_have_any_pelts_to_sell'), 'error')
        end
    end
end)
