...
Citizen.CreateThread(function()
    for mining, v in pairs(Config.MiningLocations) do
        exports['qb-core']:createPrompt(v.location, v.coords, QBCore.Shared.Keybinds['J'], Lang:t('menu.start') .. v.name, {
            type = 'client',
...
-- start mining
RegisterNetEvent('qb-mining:client:StartMining')
AddEventHandler('qb-mining:client:StartMining', function()
    local player = PlayerPedId()
    local hasItem = QBCore.Functions.HasItem('pickaxe', 1)
    if miningstarted == false then
        if hasItem then
            local randomNumber = math.random(1,100)
            if randomNumber > 95 then -- 10% chance of pickace breaking
                TriggerServerEvent('qb-mining:server:removeitem', 'pickaxe')
            else
                ...
                TriggerEvent('qb-mining:client:MiningAnimation')

                QBCore.Functions.Progressbar("mining", "Mining...", 18000, false, true,
                {
                    ...
                    TriggerServerEvent('qb-mining:server:giveMiningReward')
            ...
            QBCore.Functions.Notify(Lang:t('error.you_dont_have_pickaxe'), 'error')
        end
    else
        QBCore.Functions.Notify(Lang:t('primary.you_are_busy_the_moment'), 'primary')
    end
end)

AddEventHandler('qb-mining:client:MiningAnimation', function()
