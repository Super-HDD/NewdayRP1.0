...
RegisterNetEvent('hud:server:GainStress', function(amount)
    ...
    if Player ~= nil and Player.PlayerData.job.name ~= 'police' then
        ...
        TriggerClientEvent('QBCore:Notify', src, Lang:t("info.getstress"), 'primary')
    end
end)
...
RegisterNetEvent('hud:server:GainThirst', function(amount)
    ...
    if Player ~= nil then
        ...
        TriggerClientEvent('QBCore:Notify', src, Lang:t("info.thirsty"), 'primary')
    end
end)
...
RegisterNetEvent('hud:server:RelieveStress', function(amount)
    ...
    if Player ~= nil then
        ...
        Player.Functions.SetMetaData('stress', newStress)
        TriggerClientEvent('hud:client:UpdateStress', src, newStress)
        TriggerClientEvent('QBCore:Notify', src, Lang:t("info.relaxing"), 'primary')
    end
end)

-- count telegrams for player
QBCore.Functions.CreateCallback('hud:server:getTelegramsAmount', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
