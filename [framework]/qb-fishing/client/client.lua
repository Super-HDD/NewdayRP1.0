...
RegisterNetEvent('qb-fishing:client:usebait')
AddEventHandler('qb-fishing:client:usebait', function(UsableBait)
Citizen.CreateThread(function()
        ...
        if weaponName ~= "WEAPON_FISHINGROD" then
            QBCore.Functions.Notify(Lang:t('error.you_need_use_your_fishing_rod_first'), 'error')
            return
        end
        TriggerServerEvent('qb-fishing:server:removeBaitItem', currentLure)
        while fishing do
            Wait(0)
                ...
                if FISHING_GET_MINIGAME_STATE() == 7 then
                    ...
                    if GetGameTimer() >= nextAttTime then
                        ...
                        if probabilidadePuxar > 0.8 or probabilidadePuxar < 0.2 then -- soltar linha
                            ...
                            local r = exports["qb-fishing"]:VERTICAL_PROBE(x, y,  z, 1)
                            local valid, height = r[1], r[2]
                                                        
                        -- import from ptfx on qb-fishing c# version
                ...
                if FISHING_GET_MINIGAME_STATE() == 12 then
                    if IsControlJustPressed(0, GetHashKey("INPUT_ATTACK")) then
                        if fishing then
                            ...
                            TriggerServerEvent("qb-fishing:FishToInventory", fishModel, fishWeight)
...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if FISHING_GET_MINIGAME_STATE() == 1 then
            ...
            PromptSetActiveGroupThisFrame(fishing_data.prompt_prepare_fishing.group, CreateVarString(10, "LITERAL_STRING", Lang:t('准备开始钓鱼')))
        elseif FISHING_GET_MINIGAME_STATE() == 6 then
            t = 4

            PromptSetActiveGroupThisFrame(fishing_data.prompt_waiting_hook.group, CreateVarString(10, "LITERAL_STRING", Lang:t('您正在钓鱼，享受这段时光')))
        elseif FISHING_GET_MINIGAME_STATE() == 7 then
            fishing_data.fish.weight = FISHING_GET_F_(8)
            t = 4

            PromptSetActiveGroupThisFrame(fishing_data.prompt_hook.group, CreateVarString(10, "LITERAL_STRING", Lang:t('鱼上钩了')))
        elseif FISHING_GET_MINIGAME_STATE() == 12 then
            if fishs[GetEntityModel(FISHING_GET_FISH_HANDLE())] then
                t = 4

                PromptSetActiveGroupThisFrame(fishing_data.prompt_finish.group, CreateVarString(10, "LITERAL_STRING",Lang:t('名字')..": "..fishs[GetEntityModel(FISHING_GET_FISH_HANDLE())] .." // "..Lang:t('重量')..": "..string.format("%.2f%%", (fishing_data.fish.weight * 54.25)):gsub("%%", "").."Kg"))
            end
        end
...
function GET_TASK_FISHING_DATA()
    local r = exports["qb-fishing"]:GET_TASK_FISHING_DATA_EXTRA()
    hasMinigameOn = r[1]
    local outAsInt = r[2]
...
function SET_TASK_FISHING_DATA()
    if fishing_minigame_struct.f_0 ~= nil then
        exports["qb-fishing"]:SET_TASK_FISHING_DATA_EXTRA(fishing_minigame_struct)
    end
end
        ...
        if holding then
            for k, _ in pairs(Config.fishData) do
                ...
                if tonumber(heldModel) == tonumber(model) then
                    ...
                    if deleted then
                        TriggerServerEvent('qb-fishing:FishToInventory', model, 0)
