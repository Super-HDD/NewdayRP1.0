...
-- settings
Config.Difficulty = 0.1 -- use -1 for testing
Config.ReelSpeed = 0.1
Config.Debug = false

--1. `Config.Difficulty` 表示钓鱼的难度级别。您可以将其设置为数字值，较高的值代表更高的难度，较低的值代表更容易。使用-1可以启用测试模式，不受难度的影响。
--2. `Config.ReelSpeed` 表示钓鱼卷线的速度。较低的值会使卷线速度变慢，而较高的值会使卷线速度加快。
--3. `Config.Debug` 用于启用或禁用调试模式。设置为`true`时，将显示钓鱼过程中的调试信息，设置为`false`时将禁用调试信息。
