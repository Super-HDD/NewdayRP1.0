...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    isLoggedIn = true
    QBCore.Functions.TriggerCallback('qb-scoreboard:server:GetConfig', function(config) Config.IllegalActions = config end)
end)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        if IsControlJustReleased(0, QBCore.Shared.Keybinds['PGDN']) and IsInputDisabled(0) then -- PAGEDOWN
            if not scoreboardOpen then
                QBCore.Functions.TriggerCallback('qb-scoreboard:server:GetPlayersArrays', function(playerList)
                    QBCore.Functions.TriggerCallback('qb-scoreboard:server:GetActivity', function(police, medic)
