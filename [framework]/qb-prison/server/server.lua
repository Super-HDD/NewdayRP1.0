...
RegisterNetEvent('qb-prison:server:updateSentance', function(updatedTime)
    local src = source
...
-- remove player job
RegisterNetEvent('qb-prison:server:RemovePlayerJob', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
-- auto free player
RegisterNetEvent('qb-prison:server:FreePlayer', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('qb-prison:server:SaveJailItems', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
...
RegisterNetEvent('qb-prison:server:GiveJailItems', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    Wait(1000)
...
-- set criminal record
RegisterNetEvent('qb-prison:server:CheckRecordStatus', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
