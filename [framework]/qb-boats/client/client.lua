                    ...
                    if hour >= shopConfig.shopClose or hour < shopConfig.shopOpen then
                        ...
                        if (distanceShop <= shopConfig.distanceShop) and not IsPedInAnyBoat(player) then
                            ...
                            if Citizen.InvokeNative(0xC92AC953F0A982AE, CloseShops) then -- UiPromptHasStandardModeCompleted
                                ...
                                Wait(100)
                                QBCore.Functions.Notify(_U("shopClosed"), 'error', 6000)
                            end
                            ...
                            if Citizen.InvokeNative(0xC92AC953F0A982AE, OpenReturn) then -- UiPromptHasStandardModeCompleted
                                ...
                                if currentLocation == boatHome then
                                    ...
                                    if TransferAllow then
                                        ...
                                        TriggerServerEvent("qb-boats:TransferBoat", OwnedData, currentLocation, driveTransfer)
                                        ReturnBoat(shopId)
                                        QBCore.Functions.Notify(_U("your") .. boatName .. _U("available"), 'success', 6000)
                                    else
                                        ReturnBoat(shopId)
                                        QBCore.Functions.Notify(_U("your") .. boatName .. _U("returned") .. boatHome, 'success', 6000)
                                    end
                                end
                            end
                        end
...
function MainMenu(shopId)
    ...
    function(data, menu)
        ...
        if data.current.value == "own" then
            ...
            TriggerServerEvent('qb-boats:GetOwnedBoats', location, shopId)
        end
    end,
    function(data, menu)
        menu.close()
...
function BuyMenu(shopId)
    ...
    function(data, menu)
        ...
        if data.current.value then
            ...
            TriggerServerEvent('qb-boats:BuyBoat', buyData, location)
            menu.close()
            InMenu = false
...
RegisterNetEvent("qb-boats:OwnedBoatsMenu")
AddEventHandler("qb-boats:OwnedBoatsMenu", function(ownedBoats, shopId)
    MenuData.CloseAll()
    InMenu = true
    local elements = {}
...
function BoatMenu(shopId)
    ...
    function(data, menu)
        ...
        elseif data.current.value == "sell" then
            ...
            TriggerServerEvent('qb-boats:SellBoat', OwnedData, boatData)
            menu.close()
            InMenu = false
            ClearPedTasksImmediately(player)
            DisplayRadar(true)
        ...
        elseif data.current.value == "transfer" then
            ...
            if TransferAllow then
                ...
                QBCore.Functions.Notify(_U("transferDisabled"), 'error', 6000)
            end
        end
    end,
    function(data, menu)
    ...
    function(data, menu)
        ...
        if transferLocation then
            ...
            if transferLocation ~= location then
                TriggerServerEvent("qb-boats:TransferBoat", OwnedData, transferLocation, menuTransfer, boatData, shopName)
                menu.close()
                InMenu = false
                ClearPedTasksImmediately(PlayerPedId())
                DisplayRadar(true)
            else
                QBCore.Functions.Notify(_U("noTransfer"), 'error', 6000)
            end
        end
    end,
    function(data, menu)
        ...
        if data.current.value == "anchor" then
            if IsPedInAnyBoat(player) then
                ...
                if not isAnchored then
                    ...
                    isAnchored = true
                    QBCore.Functions.Notify(_U("anchorDown"), 'success', 5000)
                else
                    SetBoatAnchor(playerBoat, false)
                    isAnchored = false
                    QBCore.Functions.Notify(_U("anchorUp"), 'success', 5000)
                end
            end
            menu.close()
            InMenu = false
...
function SpawnBoat()
    ...
    IsBoating = true
    QBCore.Functions.Notify(_U("boatMenuTip"), 'success', 6000)
end
