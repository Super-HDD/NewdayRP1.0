...
RegisterNetEvent('qb-goldpanning:client:StartGoldPan')
AddEventHandler('qb-goldpanning:client:StartGoldPan', function()
        ...
        if panning == false then
            ...
            if canPan == true then
                ...
                if hotspot == true then
                    TriggerServerEvent('qb-goldpanning:server:hotspotreward')
                else
                    TriggerServerEvent('qb-goldpanning:server:reward')
                end
                panning = false
                canPan = false
            else
                QBCore.Functions.Notify(Lang:t('primary.you_need_the_river_to_goldpan'), 'primary')
            end
        else
            QBCore.Functions.Notify(Lang:t('error.you_are_already_goldpanning'), 'error')
        end
    else
        QBCore.Functions.Notify(Lang:t('error.you_are_mounted'), 'error')
    end
end)
