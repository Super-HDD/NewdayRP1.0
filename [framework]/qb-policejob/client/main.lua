...
--RegisterNetEvent('police:client:UpdateBlips', function(players)
    --if Config.ShowBlips then
        --if PlayerJob and (PlayerJob.name == 'police' or PlayerJob.name == 'medic') and onDuty then
            --if DutyBlips then
                --for k, v in pairs(DutyBlips) do
                    --RemoveBlip(v)
                --end
...
RegisterNetEvent('police:client:policeAlert', function(coords, text)
    QBCore.Functions.Notify(text, 'police')
    local transG = 250
...
RegisterNetEvent('police:client:SendToJail', function(time)
    ...
    ClearPedTasks(PlayerPedId())
    DetachEntity(PlayerPedId(), true, false)
    TriggerEvent('qb-prison:client:Enter', time)
end)
