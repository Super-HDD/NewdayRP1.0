...
RegisterNetEvent('police:client:SearchHorse', function()
    ...
    QBCore.Functions.TriggerCallback('police:server:GetSuspectHorse', function(data)
        ...
        if not closestPed or closestPed == -1 or closestDistance >= 2.5 then
            QBCore.Functions.Notify(Lang:t("error.nohorse_nearby"), 'error')
        ...
        if horsemodel ~= model then
            QBCore.Functions.Notify(Lang:t("error.invalid_horse"), 'error')
