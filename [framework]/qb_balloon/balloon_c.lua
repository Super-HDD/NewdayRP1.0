...
local canoe = nil
local lockZ = false
local balloonPrompts = UipromptGroup:new("热气球")
local nsPrompt = Uiprompt:new({`INPUT_VEH_MOVE_UP_ONLY`, `INPUT_VEH_MOVE_DOWN_ONLY`}, "北/南", balloonPrompts)
local wePrompt = Uiprompt:new({`INPUT_VEH_MOVE_LEFT_ONLY`, `INPUT_VEH_MOVE_RIGHT_ONLY`}, "西/东", balloonPrompts)
--local boostPrompt = Uiprompt:new(`INPUT_VEH_TRAVERSAL`, "加速", balloonPrompts) -- 因某些原因与其他提示符产生了问题
local brakePrompt = Uiprompt:new(`INPUT_VEH_BRAKE`, "减速悬停", balloonPrompts)
local lockZPrompt = Uiprompt:new(`INPUT_VEH_SHUFFLE`, "锁定高度", balloonPrompts)
local throttlePrompt = Uiprompt:new(`INPUT_VEH_FLY_THROTTLE_UP`, "上升", balloonPrompts)
...
Citizen.CreateThread(function()
    ...
    if (Vdist(coords.x, coords.y, coords.z, -5188.013, -2089.668, 17.074) < 2.0) then
            DrawTxt("使用 [~e~G~q~] 搭乘热气球.", 0.50, 0.85, 0.7, 0.7, true, 255, 255, 255, 255, true)
	   ...
	   if (Vdist(coords.x, coords.y, coords.z, -1369.349, -2357.067, 43.022) < 2.0) then
            DrawTxt("使用 [~e~G~q~] 离开热气球.", 0.50, 0.85, 0.7, 0.7, true, 255, 255, 255, 255, true)
            if IsControlJustReleased(0, 0x760A9C6F) then -- g
	...
	while true do
		if balloon then
			...
			if IsControlJustPressed(0, `INPUT_VEH_SHUFFLE`) then
				...
				if lockZ then
					lockZPrompt:setText("解锁飞行高度")
				else
					lockZPrompt:setText("锁定飞行高度")
				end
