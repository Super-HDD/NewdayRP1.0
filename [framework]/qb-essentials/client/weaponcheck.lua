    ...
    while true do
        ...
        if WeaponData ~= nil and WeaponData["name"] ~= "weapon_unarmed" then
            ...
            if weaponGroup ~= `group_thrown` then
                ...
                local hasItem = QBCore.Functions.HasItem(WeaponData["name"])
                --print(hasItem)
                if not hasItem then
                    SetCurrentPedWeapon(player, `WEAPON_UNARMED`, true)
                    RemoveAllPedWeapons(player, true, true)
                    TriggerServerEvent("qb-log:server:CreateLog", "anticheat",  Lang:t('log.weapon_removed'), "orange", '** @staff ** '..fullname..' had a weapon on them that they did not have in his inventory : anticheat has removed the weapon')
                end
            end
        end
    end
