...
function WashPrompt()
    Citizen.CreateThread(function()
        ...
        PromptSetControlAction(WashPrompt, QBCore.Shared.Keybinds['ENTER'])
...
function DrinkPrompt()
    Citizen.CreateThread(function()
        ...
        PromptSetControlAction(DrinkPrompt, QBCore.Shared.Keybinds['J'])
        str = CreateVarString(10, 'LITERAL_STRING', str)
                ...
                if IsPedOnFoot(playerPed) then
                    if IsEntityInWater(playerPed) then
                        ...
                        if PromptHasHoldModeCompleted(DrinkPrompt) then
                            TriggerEvent('qb-river:client:drink')    
                        end
                    end
                end
            end
...
-- drink action
AddEventHandler('qb-river:client:drink', function()
    local src = source
    if drink ~= 0 then
    ...
    Citizen.Wait(17000)
    TriggerServerEvent("QBCore:Server:SetMetaData", "thirst", QBCore.Functions.GetPlayerData().metadata["thirst"] + math.random(50, 100))
