...
RegisterNetEvent('qb-bandana:client:ToggleBandana')
AddEventHandler('qb-bandana:client:ToggleBandana', function()
    local ped = PlayerPedId()
    local male = IsPedMale(ped)
    local neckwear = exports['qb-clothes']:GetClothesCurrentComponentHash('neckwear')

    -- Beard of Power: https://www.nexusmods.com/skyrimspecialedition/mods/42635
    -- local beardofpower = exports['qb-appearance']:GetBodyCurrentComponentHash('beard')

    if not bandana then
        if not male then
            Citizen.InvokeNative(0xAE72E7DF013AAA61, ped, `KIT_BANDANA`, `BANDANA_ON_RIGHT_HAND`, 1, 0, -1082130432)
            Wait(700)
            Citizen.InvokeNative(0xD3A7B003ED343FD9, ped, 0xD03C1734, true, true, true)
    ...
    if not male then
        ...
        Wait(700)
        Citizen.InvokeNative(0xD710A5007C2AC539, ped, 0xD03C1734, 0)
        Wait(100)
