...
AddEventHandler('QBCore:Client:OnPlayerLoaded', function()
    ...
    QBCore.Functions.TriggerCallback('qb-wardrobe:server:getPlayerSkin', function(result)
        local ped = PlayerPedId()
        local male = IsPedMale(ped)

        if male then
            local bodyComponents = exports['qb-appearance']:GetBodyComponents()
...
RegisterNetEvent('qb-wardrobe:client:OnOffClothing', function(clothingName)
        ...
        if clothingName == name then
            if name == 'pants' then
                ...
                if clothingData['currentBoots'] == 0 then
                    clothingData['currentBoots'] = exports['qb-clothes']:GetClothesCurrentComponentHash('boots')
                ...
                if clothingData[data] == 0 then
                    clothingData[data] = exports['qb-clothes']:GetClothesCurrentComponentHash(comps)
...
-- Remove all clothing
RegisterNetEvent('qb-wardrobe:client:removeAllClothing', function()
    local ped = PlayerPedId()
    local male = IsPedMale(ped)

    TriggerServerEvent('qb-weapons:server:RemoveAllObjects')
