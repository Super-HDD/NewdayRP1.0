...
RegisterNetEvent('QBCore:client:OnPlayerLoaded', function()
    TriggerEvent("qb-notes:client:SyncNotes")
end)
...
RegisterNetEvent("qb-notes:client:SyncNotes", function(newNotes)
    if not newNotes then
        local p = promise.new()

        QBCore.Functions.TriggerCallback('qb-notes:server:SyncNotes', function(cb)
            p:resolve(cb)
        end)
    ...
    for k, v in pairs(Notes) do
        if not Props[k] and Notes[k] then
            ...
            Targets[k] = exports['qb-target']:AddCircleZone(k, vector3(v.coords.x, v.coords.y, v.coords.z-1.1), 0.5,
            {
                name            = k,
            ...
            },
            {
                options =
                {
                    {
                        type    = "server",
                        event   = "qb-notes:server:ReadNote",
                        icon    = "fas fa-receipt",
...
RegisterNetEvent("qb-notes:client:CreateNote", function()
    ...
    if dialog.note then
        ...
        TriggerServerEvent("qb-notes:server:CreateNote",
        {
...
RegisterNetEvent("qb-notes:client:ReadNote", function(data)
    local notepad = {}
    ...
    notepad[#notepad + 1] =
    {
        ...
        params                  =
        {
            event               = "qb-notes:client:DestroyNote",
            args                = data
        }
    }
...
RegisterNetEvent("qb-notes:client:DestroyNote", function(data)
    local ped = PlayerPedId()
    ...
    TriggerServerEvent("qb-notes:server:DestroyNote", data.id)
end)
