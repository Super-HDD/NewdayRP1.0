...
function OpenClothingMenu()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'clothing_store_menu',
            {title = 'Clothes', subtext = 'NDRP西部拓荒角色扮演中文社区', align = 'top-left', elements = elements}, function(data, menu)
        if data.current.value ~= "save" then
            ...
            menu.close()
            local output = nil
            AddTextEntry('FMMC_MPM_NA', "服装名称:")
            DisplayOnscreenKeyboard(1, "FMMC_MPM_NA", "", "输入名称", "", "", "", 30)
            ...
            saveOutfit = true
            TriggerServerEvent("qb-clothes:Save", ClothesCache, output, CurrentPrice)
            OldClothesCache = {}
        end
...
RegisterNetEvent('qb-clothes:OpenClothingMenu')
AddEventHandler('qb-clothes:OpenClothingMenu', function(ClothesComponents)
...
function Change(id, category, change_type)
    if id < 1 then
        ...
        if category == "pants" or category == "boots" then
            NativeSetPedComponentEnabled(PlayerPedId(), exports['qb-appearance']:GetBodyCurrentComponentHash("BODIES_LOWER"), false, true, true)
        end
        if category == "shirts_full" then
            NativeSetPedComponentEnabled(PlayerPedId(), exports['qb-appearance']:GetBodyCurrentComponentHash("BODIES_UPPER"), false, true, true)
        end
    else
        if IsPedMale(PlayerPedId()) then
...
RegisterNetEvent('qb-clothes:ApplyClothes')
AddEventHandler('qb-clothes:ApplyClothes', function(ClothesComponents, Target)
    Citizen.CreateThread(function()
...
RegisterNetEvent('qb-clothes:OpenOutfits')
AddEventHandler('qb-clothes:OpenOutfits', function()
    TriggerServerEvent('qb-clothes:getOutfits')
end)

RegisterNetEvent('qb-clothes:putInTable')
AddEventHandler('qb-clothes:putInTable', function(outfit)
    Outfits_tab = {}
...
function Outfits()
    ...
    if Outfits_tab ~= nil then
        for j, z in pairs(Outfits_tab) do
            table.insert(elements_outfits, {label = Outfits_tab[j].name, value = Outfits_tab[j].name, desc = "选择衣柜保存的套装"})
        end
    end
    MenuData.CloseAll()
    MenuData.Open('default', GetCurrentResourceName(), 'outfits_menu',
        {title = 'Clothes', subtext = 'NDRP西部拓荒角色扮演中文社区', align = 'top-left', elements = elements_outfits}, function(data, menu)
...
function OutfitsManage(outfit)
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'outfits_menu_manage',
        {title = 'Clothes', subtext = 'NDRP西部拓荒角色扮演中文社区', align = 'top-left', elements = elements_outfits_manage}, function(data, menu)
        menu.close()
        TriggerServerEvent('qb-clothes:' .. data.current.value, outfit)
    end, function(data, menu)
        Outfits()
    end)
end
        ...
        for k, v in pairs(Config.Zones) do
            ...
            if dist < 2 then
                ...
                if IsControlJustReleased(0, Config.OpenKey) then
                    TriggerEvent('qb-horses:client:FleeHorse')
