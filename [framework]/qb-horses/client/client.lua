...
RegisterNetEvent('qb-horses:client:custShop', function()
    ...
    if horsePed ~= 0 then
        ...
        if #(pcoords - coords) <= 30.0 then
            ...
            QBCore.Functions.Notify(Lang:t('error.horse_too_far'), 'error', 7500)
        end 
    else 
        QBCore.Functions.Notify('未检测到马', 'error', 7500)
    end
end)
...
RegisterCommand('sethorsename',function()
    ...
    TriggerServerEvent('qb-horses:renameHorse', input.realinput)
end)
    ...
    for k=1, #Config.StableZones do
        ...
        Zones[k]:onPlayerInOut(function(isPointInside)
            if isPointInside then
                ...
                TriggerEvent('qb-horses:client:triggerStable', zonename)
                if Config.TargetHelp == true then
                    exports['qb-core']:DrawText('使用[左-ALT]来定位', 'left')
                end
            else
                inStableZone = false
                TriggerEvent('qb-horses:client:distroyStable')
                if Config.TargetHelp == true then
...
RegisterNetEvent('qb-horses:client:triggerStable', function(zone)
                        ...
                        options = {
                            {
                                ...
                                action = function(newnames)
                                    local input = lib.inputDialog('给您的爱马取个名', {'名字',  {type = 'select', label = '选择性别',  options = {
                                        { value = 'male', label = '雄性'},
                                        { value = 'female', label = '雌性' },
                            
                                    }, default = 'male', clearable = false, required = true},})
                                    
                                    if input  then
                                        TriggerServerEvent('qb-horses:server:BuyHorse', n.price, n.model, input[1], input[2])
                                    else
                                        return
                                    end
                                end
                ...
                options = {
                    {
                        ...
                        targeticon = "fas fa-eye",
                        action = function()
                            TriggerEvent("qb-horses:client:menu")
                        end
                    },
                    {
                        icon = "fas fa-horse-head",
                        ...
                        targeticon = "fas fa-eye",
                        action = function()
                            TriggerEvent("qb-horses:client:storehorse")
                        end
                    },
                    {
                        icon = "fas fa-horse-head",
                        ...
                        targeticon = "fas fa-eye",
                        action = function()
                            TriggerEvent("qb-horses:client:MenuDel")
                        end
                    },
                    {
                        icon = "fas fa-horse-head",
                        ...
                        targeticon = "fas fa-eye",
                        action = function()
                        TriggerEvent('qb-horses:client:custShop')
                        end
                    },
                    {
                        icon = "fas fa-horse-head",
                        ...
                        targeticon = "fas fa-eye",
                        action = function()
                        TriggerEvent('qb-horses:client:tradehorse')
                        end
                    },
                    {
                        icon = "fas fa-award",
                        ...
                        targeticon = "fas fa-eye",
                        action = function()
                        TriggerEvent('qb-horsetrainer:client:OpenTrainerShop')
                        end
                    },
                    {
                        ...
                        targeticon = "fas fa-eye",
                        action = function()
                        TriggerEvent('qb-horses:client:OpenHorseShop')
                        end
                    },
                },
                distance = 2.5,
...
RegisterNetEvent('qb-horses:client:distroyStable', function()
...
local function TradeHorse()
    QBCore.Functions.TriggerCallback('qb-horses:server:GetActiveHorse', function(data,newnames)
        if horsePed ~= 0 then
            local player, distance = QBCore.Functions.GetClosestPlayer()
            if player ~= -1 and distance < 1.5 then
                local playerId = GetPlayerServerId(player)
                local horseId = data.horseid
                TriggerServerEvent('qb-horses:server:TradeHorse', playerId, horseId)
                QBCore.Functions.Notify(Lang:t('success.horse_traded'), 'success', 7500)
            else
                QBCore.Functions.Notify(Lang:t('error.no_nearby_player'), 'success', 7500)
            end
        end
    end)
end
    ...
    QBCore.Functions.TriggerCallback('qb-horses:server:GetActiveHorse', function(data)
        if (data) then
            ...
            if Config.SpawnOnRoadOnly and not onRoad then
                QBCore.Functions.Notify(Lang:t('error.near_road'), 'error')
...
function CustomHorse()
    ...
    MenuData.Open('default', GetCurrentResourceName(), 'horse_menu',
    {
        ...
        TriggerEvent('qb-horses:client:SaveHorseComponents', data.current.category, data.current.value)
    end,
    function(_, menu)
        menu.close()

        TriggerEvent('qb-horses:closeMenu')
    end)
end
...
AddEventHandler('qb-horses:client:SaveHorseComponents', function(category, value)
    if not horsePed or horsePed == 0 then
        QBCore.Functions.Notify(Lang:t('error.no_horse_found'), 'error')
    ...
    if category == 'blankets' then
        if not value or value == 0 then
            ...
            TriggerServerEvent('qb-horses:server:SaveBlankets', 0)
        ...
        TriggerServerEvent('qb-horses:server:SaveBlankets', blanketsHash)
    elseif category == 'saddles' then
        if not value or value == 0 then
            RemoveHorseComponents(0xBAA7E618)

            TriggerServerEvent('qb-horses:server:SaveSaddles', 0)
        ...
        TriggerServerEvent('qb-horses:server:SaveSaddles', saddlesHash)
    elseif category == 'horns' then
        if not value or value == 0 then
            RemoveHorseComponents(0x05447332)

            TriggerServerEvent('qb-horses:server:SaveHorns', 0)
        ...
        TriggerServerEvent('qb-horses:server:SaveHorns', hornsHash)
    elseif category == 'saddlebags' then
        if not value or value == 0 then
            RemoveHorseComponents(0x80451C25)

            TriggerServerEvent('qb-horses:server:SaveSaddlebags', 0)
        ...
        TriggerServerEvent('qb-horses:server:SaveSaddlebags', saddlebagsHash)
    elseif category == 'stirrups' then
        if not value or value == 0 then
            RemoveHorseComponents(0xDA6DADCA)

            TriggerServerEvent('qb-horses:server:SaveStirrups', 0)
        ...
        TriggerServerEvent('qb-horses:server:SaveStirrups', stirrupsHash)
    elseif category == 'bedrolls' then
        if not value or value == 0 then
            Citizen.InvokeNative(0xD710A5007C2AC539, horsePed, 0xEFB31921, 0)
            Citizen.InvokeNative(0xCC8CA3E88256E58F, horsePed, 0, 1, 1, 1, 0)

            TriggerServerEvent('qb-horses:server:SaveBedrolls', 0)
        ...
        TriggerServerEvent('qb-horses:server:SaveBedrolls', bedrollsHash)
    elseif category == 'tails' then
        if not value or value == 0 then
            Citizen.InvokeNative(0xD710A5007C2AC539, horsePed, 0xA63CAE10, 0)
            Citizen.InvokeNative(0xCC8CA3E88256E58F, horsePed, 0, 1, 1, 1, 0)

            TriggerServerEvent('qb-horses:server:SaveTails', 0)
        ...
        TriggerServerEvent('qb-horses:server:SaveTails', tailsHash)
    elseif category == 'manes' then
        if not value or value == 0 then
            Citizen.InvokeNative(0xD710A5007C2AC539, horsePed, 0xAA0217AB, 0)
            Citizen.InvokeNative(0xCC8CA3E88256E58F, horsePed, 0, 1, 1, 1, 0)

            TriggerServerEvent('qb-horses:server:SaveManes', 0)
        ...
        TriggerServerEvent('qb-horses:server:SaveManes', manesHash)
    elseif category == 'masks' then
        if not value or value == 0 then
            Citizen.InvokeNative(0xD710A5007C2AC539, horsePed, 0xD3500E5D, 0)
            Citizen.InvokeNative(0xCC8CA3E88256E58F, horsePed, 0, 1, 1, 1, 0)

            TriggerServerEvent('qb-horses:server:SaveMasks', 0)
        ...
        TriggerServerEvent('qb-horses:server:SaveMasks', masksHash)
    elseif category == 'mustaches' then
        if not value or value == 0 then
            Citizen.InvokeNative(0xD710A5007C2AC539, horsePed, 0x30DEFDDF, 0)
            Citizen.InvokeNative(0xCC8CA3E88256E58F, horsePed, 0, 1, 1, 1, 0)

            TriggerServerEvent('qb-horses:server:SaveMustaches', 0)
        ...
        TriggerServerEvent('qb-horses:server:SaveMustaches', mustachesHash)
    end
end)

----------------------------------------------------------------------------------------------------

RegisterNetEvent('qb-horses:closeMenu', function()
...
RegisterNetEvent('qb-horses:client:SpawnHorse', function(data)
    HorseId = data.player.id
    TriggerServerEvent("qb-horses:server:SetHoresActive", data.player.id)
    QBCore.Functions.Notify(Lang:t('success.horse_active'), 'success', 7500)
end)

AddEventHandler('qb-horses:client:FleeHorse', function()
    if horsePed then
        getControlOfEntity(horsePed)
...
RegisterNetEvent("qb-horses:client:storehorse", function(data)
    if (horsePed ~= 0) then
        TriggerServerEvent("qb-horses:server:SetHoresUnActive", HorseId)
        QBCore.Functions.Notify(Lang:t('success.storing_horse'), 'success', 7500)
        Flee()
        HorseCalled = false
    end
end)

RegisterNetEvent("qb-horses:client:tradehorse", function(data)
    QBCore.Functions.TriggerCallback('qb-horses:server:GetActiveHorse', function(data,newnames)
        if (horsePed ~= 0) then
            TradeHorse()
            Flee()
            ...
            QBCore.Functions.Notify(Lang:t('error.no_horse_out'), 'error', 7500)
        end
    end)
end)

RegisterNetEvent('qb-horses:client:menu', function()
    local options = {
       
    }
    QBCore.Functions.TriggerCallback('qb-horses:server:GetHorse', function(cb)
        for _, v in pairs(cb) do
            options[#options + 1] = {
                ...
                icon = "fa-solid fa-circle-user",
                    ...
                    event = "qb-horses:client:SpawnHorse",
                    args = {
                        player = v,
                        active = 1
                    }
        ...
        lib.registerContext({
            id = 'horse_menu',
            title = Lang:t('menu.my_horses'),
            options = options
        })
        lib.showContext('horse_menu')
    end)
end)

RegisterNetEvent('qb-horses:client:MenuDel', function()
    local GetHorse = {
        {
    ...
    QBCore.Functions.TriggerCallback('qb-horses:server:GetHorse', function(cb)
        for _, v in pairs(cb) do
            GetHorse[#GetHorse + 1] = {
                ...
                icon = "fa-solid fa-circle-user",
                    ...
                    event = "qb-horses:client:MenuDelC",
                    args = {}
        ...
        lib.registerContext({
            id = 'Get_Horse',
            title = Lang:t('menu.sell_horses'),
            
            options = GetHorse
        })
        lib.showContext('Get_Horse')
    end)
end)


RegisterNetEvent('qb-horses:client:MenuDelC', function(data)
    local GetHorsee = {
        
    }
    QBCore.Functions.TriggerCallback('qb-horses:server:GetHorse', function(cb)
        for _, v in pairs(cb) do
            GetHorsee[#GetHorsee + 1] = {
                title = v.name,
                description = Lang:t('menu.sell_warning'),
                icon = "fa-solid fa-circle-user",
                
                    event = "qb-horses:client:DeleteHorse",
                    args = {
                        player = v,
                        active = 1
                    }
        ...
        lib.showContext('Get_Horsee')
    end)
end)

RegisterNetEvent('qb-horses:client:DeleteHorse', function(data)
    QBCore.Functions.Notify(Lang:t('success.horse_sold'), 'success', 7500)
    TriggerServerEvent("qb-horses:server:DelHores", data.player.id)
end)
...
CreateThread(function()
    while true do
        ...
        if Citizen.InvokeNative(0x91AEF906BCA88877, 0, QBCore.Shared.Keybinds['H']) then -- call horse
        ...
        if Citizen.InvokeNative(0x91AEF906BCA88877, 0, QBCore.Shared.Keybinds['HorseCommandFlee']) then -- flee horse
...
CreateThread(function()
    while true do
        ...
        if #(pcoords - hcoords) <= 1.7 and Citizen.InvokeNative(0x580417101DDB492F, 0, Config.HorseInvKey) then
            TriggerEvent('qb-horses:client:inventoryHorse')
        end
    end
end)

-- horse inventory
RegisterNetEvent('qb-horses:client:inventoryHorse', function()
    QBCore.Functions.TriggerCallback('qb-horses:server:GetActiveHorse', function(data)
        if horsePed == 0 then
            QBCore.Functions.Notify(Lang:t('error.no_horse_out'), 'error', 7500)
            return
...
RegisterNetEvent('qb-horses:client:equipHorseLantern')
AddEventHandler('qb-horses:client:equipHorseLantern', function()
    local hasItem = QBCore.Functions.HasItem('horselantern', 1)

    if not hasItem then
        QBCore.Functions.Notify(Lang:t('error.no_lantern'), 'error')
        return
    end
    ...
    if distance > 2.0 then
        QBCore.Functions.Notify(Lang:t('error.need_to_be_closer'), 'error')
        return
    end
    ...
    if lanternequiped == false then
        ...
        QBCore.Functions.Notify(Lang:t('primary.lantern_equiped'), 'horse', 3000)
        return
    end
    ...
    if lanternequiped == true then
        ...
        QBCore.Functions.Notify(Lang:t('primary.lantern_removed'), 'horse', 3000)
        return
    end
end)
...
RegisterNetEvent('qb-horses:client:equipHorseHolster')
AddEventHandler('qb-horses:client:equipHorseHolster', function()
    local hasItem = QBCore.Functions.HasItem('horseholster', 1)
    if not hasItem then
        QBCore.Functions.Notify(Lang:t('error.no_holster'), 'error')
        return
    end
    ...
    if holsterequiped == false then
        ...
        QBCore.Functions.Notify(Lang:t('primary.holster_equiped'), 'horse', 3000)
        return
    end
    ...
    if holsterequiped == true then
        ...
        QBCore.Functions.Notify(Lang:t('primary.holster_removed'), 'horse', 3000)
        return
    end
end)
...
-- player feed horse
RegisterNetEvent('qb-horses:client:playerfeedhorse')
AddEventHandler('qb-horses:client:playerfeedhorse', function(itemName)
    local pcoords = GetEntityCoords(PlayerPedId())
    local hcoords = GetEntityCoords(horsePed)
...
-- player brush horse
RegisterNetEvent('qb-horses:client:playerbrushhorse')
AddEventHandler('qb-horses:client:playerbrushhorse', function(itemName)
    local pcoords = GetEntityCoords(PlayerPedId())
    local hcoords = GetEntityCoords(horsePed)
...
-- Player revive horse
RegisterNetEvent("qb-horses:client:revivehorse")
AddEventHandler("qb-horses:client:revivehorse", function(item, data)
    local playerPed = PlayerPedId()
    ...
    if horsePed == 0 then
        QBCore.Functions.Notify('没有生成活跃的马!', 'error')
    ...
    if IsEntityDead(horsePed) then
        if distance > 1.5 then
            QBCore.Functions.Notify('你的马太远了!', 'error')
        ...
        QBCore.Functions.Progressbar("reviving-horse", "正在治疗马..", 3000, false, true,
        {
            ...
            ClearPedTasks(playerPed)
            FreezeEntityPosition(playerPed, false)
            TriggerServerEvent('qb-horses:server:revivehorse', item)
            SpawnHorse()
        end)
    else
        QBCore.Functions.Notify('马没有受伤也没有死!', 'error')
    end
end)

-------------------------------------------------------------------------------

RegisterNetEvent('qb-horses:client:OpenHorseShop')
AddEventHandler('qb-horses:client:OpenHorseShop', function()
