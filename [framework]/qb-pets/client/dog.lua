...
function newDoggo(model,name)
    ...
    Citizen.CreateThread(function()
        while Config.enable_debugger do
            ...
            DrawText(0.5,0.02,"ped生成: "..tostring(object.spawned))
            DrawText(0.5,0.04,"ped模型: "..tostring(object.model))
            DrawText(0.5,0.06,"ped位置: "..tostring(object.pos))
            DrawText(0.5,0.08,"Ped id: "..tostring(object.id))
            DrawText(0.5,0.10,"行人姓名: "..tostring(object.name))
            DrawText(0.5,0.12,"ped状态: "..tostring(object.state))
            DrawText(0.5,0.14,"ped·布利普: "..tostring(object.blip))
            DrawText(0.5,0.16,"Ped关注范围: "..tostring(object.following_range))
            DrawText(0.5,0.18,"ped漫游范围: "..tostring(object.wandering_range))
            DrawText(0.5,0.20,"ped嗅觉范围: "..tostring(object.smelling_range))
            DrawText(0.5,0.22,"pedplayer_range: "..tostring(object.player_range))
            DrawText(0.5,0.24,"Ped 计数器.休息: "..tostring(object.counters.resting))
            DrawText(0.5,0.26,"Ped volume: "..tostring(object.volume))
            DrawText(0.5,0.28,"ped狩猎名单: "..tostring(tablelength(object.huntingList)))
            DrawText(0.5,0.30,"Ped 项集: "..tostring(object.itemset))

            local txtpos = 0.32
            for k,v in pairs(object.huntingList) do
                txtpos = txtpos + 0.02
                DrawText(0.5,txtpos,"ped钥匙=".. k .." "..tostring(v))
            end
