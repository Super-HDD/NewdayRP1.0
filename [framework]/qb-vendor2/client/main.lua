...
RegisterNetEvent("qb-vendoer:client:closemenu")
AddEventHandler("qb-vendoer:client:closemenu", function()
    closeMenuFull()
end)
...
RegisterNetEvent("qb-vendoer:client:vendorMenu", function()
    --print(currentvendor)
    QBCore.Functions.TriggerCallback('qb-vendoer:server:vendorOwned', function(result)
        if result == nil then
            --print(currentvendor)
            ...
            vendorMenuFirst[#vendorMenuFirst+1] = {
                ...
                params = {
                    event = "qb-vendoer:client:vendorAchat",
                    args = currentvendor
                }
            }
            vendorMenuFirst[#vendorMenuFirst+1] = {
                ...
                txt = "",
                params = {
                    event = "qb-vendoer:client:closemenu",
                }
            }
            exports['qb-menu']:openMenu(vendorMenuFirst)
        else
            QBCore.Functions.TriggerCallback('qb-vendoer:server:vendorOwner', function(result2)
                if result2 == nil then
                    local vendorMenuFirst = {
                        {
                            header = Lang:t('menu.market'),
                    ...
                    vendorMenuFirst[#vendorMenuFirst+1] = {
                        ...
                        params = {
                            isServer = true,
                            event = "qb-vendoer:server:vendorGetShopItems",
                            args = currentvendor
                        }
                    }
                    vendorMenuFirst[#vendorMenuFirst+1] = {
                        ...
                        params = {
                            event = "qb-vendoer:client:vendorRob",
                            args = currentvendor,
                        }
                    }
                    vendorMenuFirst[#vendorMenuFirst+1] = {
                    ...
                    vendorMenuFirst[#vendorMenuFirst+1] = {
                        ...
                        params = {
                            event = "qb-vendoer:client:vendorInvReFull",
                        }
                    }
                    vendorMenuFirst[#vendorMenuFirst+1] = {
                        ...
                        params = {
                            event = "qb-vendoer:client:vendorCheckMoney",
                            args = data,
                        }
                    }
                    vendorMenuFirst[#vendorMenuFirst+1] = {
                        ...
                        icon = 'fa-solid fa-sack-dollar',
                        params = {
                            event = "qb-vendoer:client:vendorSettings",
                            args = v,
                        }
                    }
                    vendorMenuFirst[#vendorMenuFirst+1] = {
...
RegisterNetEvent("qb-vendoer:client:vendorInv", function(store_inventory, data)
    QBCore.Functions.TriggerCallback('qb-vendoer:server:vendorS', function(result)
        ...
        for k, v in pairs(store_inventory) do
            ...
            vendorMenuItem[#vendorMenuItem+1] = {
                header = "<img src=nui://qb-inventory/html/images/"..QBCore.Shared.Items[store_inventory[k].items].image.." width=20px> ‎ ‎ "..QBCore.Shared.Items[store_inventory[k].items].label,
                txt = Lang:t('menu.instock')..": "..store_inventory[k].stock.." | "..Lang:t('menu.price')..": $"..store_inventory[k].price,
                params = {
                    event = "qb-vendoer:client:vendorInvInput",
        ...
        vendorMenuItem[#vendorMenuItem+1] = {
            ...
            txt = "",
            params = {
                event = "qb-vendoer:client:vendorMenu",
            }
        }
...
RegisterNetEvent("qb-vendoer:client:vendorInvReFull", function()
            ...
            if PlayerData.items == nil then 
                ...
                for k, v in pairs(PlayerData.items) do
                    ...
                    vendorMenuInvItem[#vendorMenuInvItem+1] = {
                        header = "<img src=nui://qb-inventory/html/images/"..PlayerData.items[k].image.." width=20px> ‎ ‎ "..PlayerData.items[k].label,
                        txt = Lang:t('menu.in_inv').." : "..PlayerData.items[k].amount,
                        params = {
                            event = "qb-vendoer:client:vendorInvReFillInput",
...
RegisterNetEvent("qb-vendoer:client:vendorCheckMoney", function()
    --print(1)
    QBCore.Functions.TriggerCallback('qb-vendoer:server:vendorGetMoney', function(checkmoney)
        QBCore.Functions.TriggerCallback('qb-vendoer:server:vendorS', function(result)
            --print(checkmoney)
            ...
            vendorMenuMoney[#vendorMenuMoney+1] = {
                ...
                params = {
                    event = "qb-vendoer:client:vendorWithdraw",
                    args =  checkmoney,
                }
            }
            vendorMenuMoney[#vendorMenuMoney+1] = {
...
RegisterNetEvent("qb-vendoer:client:vendorAchat", function(currentvendor)
    ...
    QBCore.Functions.TriggerCallback('qb-vendoer:server:vendorS', function(result)
        ...
        vendorAchat[#vendorAchat+1] = {
            ...
            params = {
                event = "qb-vendoer:client:vendorBuyEtal",
...
RegisterNetEvent("qb-vendoer:client:vendorSettings", function()
    QBCore.Functions.TriggerCallback('qb-vendoer:server:vendorS', function(result)
        ...
        vendorSettings[#vendorSettings+1] = {
            ...
            params = {
                event = "qb-vendoer:client:vendorName",
                args = "",
            }
        }
        vendorSettings[#vendorSettings+1] = {
            ...
            params = {
                event = "qb-vendoer:client:vendorGiveBusiness",
                args = "",
            }
        }
        vendorSettings[#vendorSettings+1] = {
...
RegisterNetEvent("qb-vendoer:client:vendorGiveBusiness", function()
    ...
    if dialoggive ~= nil then
            TriggerServerEvent('qb-vendoer:server:vendorGiveBusiness', currentvendor, dialoggive.tocid)
    end
end)

RegisterNetEvent("qb-vendoer:client:vendorName", function()
    ...
    if ChangeName ~= nil then 
        TriggerServerEvent('qb-vendoer:server:vendorName', currentvendor, ChangeName.name)
    end
end)

RegisterNetEvent("qb-vendoer:client:vendorWithdraw", function(checkmoney)
    ...
    if Withdraw ~= nil then 
        TriggerServerEvent('qb-vendoer:server:vendorWithdraw', currentvendor, Withdraw.qt)
    end
end)

RegisterNetEvent("qb-vendoer:client:vendorInvReFillInput", function(data)
    local name = data
    ...
    QBCore.Functions.GetPlayerData(function(PlayerData)
        for k, v in pairs(PlayerData.items) do
            if amount >= tonumber(Refill.qt) then
                TriggerServerEvent('qb-vendoer:server:vendorInvReFill', currentvendor, name, Refill.qt, Refill.qtp)
            else
...
RegisterNetEvent("qb-vendoer:client:vendorInvInput", function(data)
    ...
    if stock >= tonumber(howmany.qt) then
        TriggerServerEvent('qb-vendoer:server:vendorPurchaseItem', currentvendor, name, howmany.qt)
    else
        QBCore.Functions.Notify(("Invalid Amount"), 'error')
    end
end)
...
RegisterNetEvent("qb-vendoer:client:vendorBuyEtal")
AddEventHandler("qb-vendoer:client:vendorBuyEtal", function()
    --print(currentvendor)
    --print(price)
    TriggerServerEvent('qb-vendoer:server:vendorBuyEtal', currentvendor, price)
end)
...
AddEventHandler("Stores:ReturnStoreItems", function(data, data2)
    ...
    Wait(100)
    TriggerEvent('qb-vendoer:client:vendorInv', store_inventory, data2)
end)
...
RegisterNetEvent("qb-vendoer:client:vendorRob", function()
                ...
                for i, x in pairs(peds) do
                    if HasEntityClearLosToEntityInFront(me, peds[i], 19) and not IsPedDeadOrDying(peds[i]) and GetDistanceBetweenCoords(GetEntityCoords(me), GetEntityCoords(peds[i]), true) <= 4.0 then
                        ...
                        if randomNumber <= Config.ChanceFail then
                            ...
                            TriggerServerEvent("qb-vendoer:server:vendorRob", currentvendor)
                        end
                    end
                end
            else
...
CreateThread(function()
    for k, v in pairs(Config.Market) do
        exports['qb-core']:createPrompt(Config.Market[k], Config.Market[k].coords, 0xF3830D8E, Lang:t('other.prompt'), {
            type = 'client',
            event = 'qb-vendoer:client:vendorMenuPrompt'
        })
    end
end)

RegisterNetEvent('qb-vendoer:client:vendorMenuPrompt', function()
    for k, v in pairs(Config.Market) do
        local PutOutDist = #(GetEntityCoords(PlayerPedId()) - Config.Market[k].coords)
        if PutOutDist <= 4 then
            if not IsPedInAnyVehicle(PlayerPedId()) then
                currentvendor = k
                TriggerEvent('qb-vendoer:client:vendorMenu')
            end
        end
    end
end)
...
-------------------------------------------------------------------------------------------
--[[
RegisterNetEvent("qb-vendoer:server:vendorDbInv")
AddEventHandler("qb-vendoer:server:vendorDbInv", function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        for k, v in pairs(PlayerData.items) do 
...
RegisterCommand('debug_inv', function()
    TriggerEvent("qb-vendoer:server:vendorDbInv")
end)

RegisterCommand('debug_input', function()
    TriggerEvent("qb-vendoer:client:vendorName")
end)
]]--
