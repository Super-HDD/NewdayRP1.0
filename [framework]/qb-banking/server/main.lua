...
QBCore.Functions.CreateUseableItem('moneyclip', function(source, item)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    ...
    if Player.Functions.RemoveItem(item.name, 1, item.slot) then
        ...
        QBCore.Functions.Notify(src, '你已经得到了$'..amount..'这个钱夹里的现金!', 'success', 3000)
    end
end)

QBCore.Commands.Add('moneyclip', 'Make Money Clip', {{ name = 'amount', help = 'How much money do you want convert?' }}, true, function(source, args)
    local src = source
    local args1 = num(args[1])

    if args1 <= 0 then
        QBCore.Functions.Notify(src, 'Please enter the correct amount!', 'error', 3000)
    ...
    if money and money >= args1 then
        if Player.Functions.RemoveMoney('cash', args1, 'give-money') then
            ...
            QBCore.Functions.Notify(src, 'You\'ve just converted $'..args1..' cash into a Money Clip!', 'success', 3000)
        end
    end
end, 'user')