...
RegisterNetEvent('qb-weapons:client:AutoDualWield', function()
    ...
    if not alreadyUsed and not UsedWeapons[tonumber(hash)] then
        ...
        if weaponName == 'weapon_bow' or weaponName == 'weapon_bow_improved' then
            if weaponData.info.ammo == nil or weaponData.info.ammo == 0 then
                local hasItem = QBCore.Functions.HasItem('ammo_arrow', 1)
                if hasItem then
                    weaponData.info.ammo = Config.AmountArrowAmmo
                    weaponData.info.ammoclip = Config.AmountArrowAmmo
                    TriggerServerEvent('qb-weapons:server:removeWeaponAmmoItem', 'ammo_arrow')
                else
                    weaponData.info.ammo = 0
                    weaponData.info.ammoclip = 0
                    QBCore.Functions.Notify(Lang:t('error.no_arrows_your_inventory_load'), 'error')
                end
            end
        ...
        elseif string.find(weaponName, 'thrown') then
            ...
            TriggerServerEvent('qb-weapons:server:removeWeaponItem', weaponName, 1)
        else
        ...
        TriggerServerEvent('qb-weapons:server:LoadComponents', wepSerial, hash)
        ...
        TriggerServerEvent('qb-weapons:server:SaveAmmo', weaponData.info.serie, ammo, ammoclip)
...
-- Components Loader
RegisterNetEvent("qb-weapon:client:LoadComponents")
AddEventHandler("qb-weapon:client:LoadComponents", function(components, hash)
    Wait(500)
...
-- load ammo
RegisterNetEvent('qb-weapons:client:AddAmmo', function(ammotype, amount, ammo)
    local ped = PlayerPedId()
    ...
    if weapon == -1569615261 then
        QBCore.Functions.Notify(Lang:t('error.you_are_not_holding_weapon'), 'error')
        return
    end
    ...
    if not valid_ammo then
        QBCore.Functions.Notify(Lang:t('error.wrong_ammo_for_weapon'), 'error')
        return
    end
    ...
    if total + math.floor(amount * 0.5) < max_ammo then
        if QBCore.Shared.Weapons[weapon] then
            ...
            TriggerServerEvent('qb-weapons:server:removeWeaponAmmoItem', ammo_type)
        end
        return
    end

    QBCore.Functions.Notify(Lang:t('error.max_mmo_capacity'), 'error')
end)
...
CreateThread(function()
    while true do
        ...
        if currentSerial ~= nil and heldWeapon ~= -1569615261 then
            TriggerServerEvent('qb-weapons:server:SaveAmmo', currentSerial, tonumber(getammo), tonumber(getammoclip))
        end
    end
end)
