...
RegisterNetEvent('qb-bathing:client:StartBath')
AddEventHandler('qb-bathing:client:StartBath', function(town)
    inbath = true 
    if Config.BathingZones[town] then
        ...
        TriggerEvent("qb-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { PlayerPedId(), "Script_Mini_Game_Bathing_Regular", `CLIPSET@MINI_GAMES@BATHING@REGULAR@ARTHUR`, `DEFAULT`, "BATHING" })
        TriggerEvent("qb-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { rag, "Script_Mini_Game_Bathing_Regular", `CLIPSET@MINI_GAMES@BATHING@REGULAR@RAG`, `DEFAULT`, "BATHING" })    
...
ExitBathing = function(animscene, town, cam)
    ...
    TriggerServerEvent("qb-bathing:server:setBathAsFree", town)
...
RegisterNetEvent('qb-bathing:client:StartDeluxeBath')
AddEventHandler('qb-bathing:client:StartDeluxeBath', function(animscene, town, cam)
    if not IsPedMale(PlayerPedId()) then
        ...
        TriggerEvent("qb-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { PlayerPedId(), "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@DELUXE@ARTHUR`, `DEFAULT`, "BATHING" })
        TriggerEvent("qb-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { BathingPed, "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@DELUXE@MAID`, `DEFAULT`, "BATHING" })    
...
RegisterNetEvent('qb-bathing:client:HideDeluxePrompt')
AddEventHandler('qb-bathing:client:HideDeluxePrompt', function()
...
ExitPremiumBath = function(animscene, town, cam, disableScrub)
    ...
    TriggerEvent("qb-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { PlayerPedId(), "Script_Mini_Game_Bathing_Regular", `CLIPSET@MINI_GAMES@BATHING@REGULAR@ARTHUR`, `DEFAULT`, "BATHING" })
    TriggerEvent("qb-bathing:TASK_MOVE_NETWORK_BY_NAME_WITH_INIT_PARAMS", { BathingPed, "Script_Mini_Game_Bathing_Deluxe", `CLIPSET@MINI_GAMES@BATHING@REGULAR@MAID`, `DEFAULT`, "BATHING" })    
...
UndressCharacter = function()
    ...
    EquippedWeapons = exports['qb-weapons']:EquippedWeapons()

    for i = 1, #EquippedWeapons do
        RemoveWeaponFromPed(ped, EquippedWeapons[i])
    end
    TriggerEvent('qb-wardrobe:client:removeAllClothing')
end
...
Action = function(name, p1, p2, p3)
    ...
    if (name == "START_BATHING") then
        TriggerServerEvent("qb-bathing:server:canEnterBath", p1)
    elseif (name == "REQUEST_DELUXE_BATHING") then
        TriggerServerEvent("qb-bathing:server:canEnterDeluxeBath", p1 , p2 , p3)
