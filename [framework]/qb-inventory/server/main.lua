	...
	elseif itemData.useable then
        if itemData.info.quality > 0 then
            ...
            if itemInfo['delete'] and RemoveItem(src, itemData.name, 1, slot) then
                ...
                QBCore.Functions.Notify(src, QBCore.Shared.Items[itemData.name].label..' has decayed and you can\'t use it anymore!', 'error')
            else
                QBCore.Functions.Notify(src, 'You can\'t use this item!', 'error')
            end
        end
	end
end)
...
QBCore.Commands.Add("rob", "Rob Player", {}, false, function(source, _)
	TriggerClientEvent("qb-looting:client:RobPlayer", source)
end)
	...
	if Player then
			if itemData then
				...
				if AddItem(id, itemData["name"], amount, false, info) then
					QBCore.Functions.Notify(source, Lang:t("notify.yhg") ..GetPlayerName(id).." "..amount.." "..itemData["label"].. "", "success")
				else
