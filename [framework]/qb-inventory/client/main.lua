...
CreateThread(function()
    while true do
        Wait(10)
        if IsControlJustReleased(0, QBCore.Shared.Keybinds['I']) then -- key open inventory I
...
CreateThread(function()
    while true do
        Wait(0)
        DisableControlAction(0, QBCore.Shared.Keybinds['1'])
        DisableControlAction(0, QBCore.Shared.Keybinds['2'])
        DisableControlAction(0, QBCore.Shared.Keybinds['3'])
        DisableControlAction(0, QBCore.Shared.Keybinds['4'])
        DisableControlAction(0, QBCore.Shared.Keybinds['5'])
        DisableControlAction(0, QBCore.Shared.Keybinds['Z'])
        if IsDisabledControlPressed(0, QBCore.Shared.Keybinds['1']) and IsInputDisabled(0) then  -- 1  slot
        ...
        if IsDisabledControlPressed(0, QBCore.Shared.Keybinds['2']) and IsInputDisabled(0) then  -- 2 slot
			if not PlayerData.metadata["isdead"] and not PlayerData.metadata["ishandcuffed"] then
				TriggerServerEvent("inventory:server:UseItemSlot", 2)
			end
        end

        if IsDisabledControlPressed(0, QBCore.Shared.Keybinds['3']) and IsInputDisabled(0) then -- 3 slot
			if not PlayerData.metadata["isdead"] and not PlayerData.metadata["ishandcuffed"] then
				TriggerServerEvent("inventory:server:UseItemSlot", 3)
			end
        end

        if IsDisabledControlPressed(0, QBCore.Shared.Keybinds['4']) and IsInputDisabled(0) then  -- 4 slot
			if not PlayerData.metadata["isdead"] and not PlayerData.metadata["ishandcuffed"] then
				TriggerServerEvent("inventory:server:UseItemSlot", 4)
			end
        end

        if IsDisabledControlPressed(0, QBCore.Shared.Keybinds['5']) and IsInputDisabled(0) then -- 5 slot
			if not PlayerData.metadata["isdead"] and not PlayerData.metadata["ishandcuffed"] then
				TriggerServerEvent("inventory:server:UseItemSlot", 5)
			end
        end

        if IsDisabledControlJustPressed(0, QBCore.Shared.Keybinds['Z']) and IsInputDisabled(0) then -- z  Hotbar
            isHotbar = not isHotbar
            ToggleHotbar(isHotbar)
        end
    end
