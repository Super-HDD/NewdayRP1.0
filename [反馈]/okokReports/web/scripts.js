...
function viewFeedbackWindow(feedbackid) {
	...
	if (data.category === 'player_report') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-user-alt"></i> 玩家报告</span>`)
	} else if (data.category === 'question') {
		feedbackWcard.find(".feedback_category").html(`<span><i class="fas fa-question"></i> 问题</span>`)
	} else {
...
function reloadFeedbackTable() {
	...
	$.each(FeedbackTable, function(id, feedback) {
		...
		if (feedback.category === 'player_report') {
			category = "<span><i class='fas fa-user-alt'></i> 玩家报告</span>"
		} else if (feedback.category === 'question') {
			category = "<span><i class='fas fa-question'></i> 问题</span>"
		} else {
		...
		if (feedback.concluded != true) {
			feedbackTable.append(`
				<tr class="table-row" feedbackID="${id}">
					...
					<td><button type="button" class="btn btn-view"><i class="fas fa-eye"></i> 查看</button></td>
				</tr>`
			)
		}
	});
	...
	if ($(".feedbackTable .table").children().length <= 0) {
		$(".feedbackTable .feedback_subtitle").html("不错！ 您现在可以放松并喝一口.");
		...
		$(".feedback_subtitle").html("嘿！ 看来您还有一些工作要做...");
