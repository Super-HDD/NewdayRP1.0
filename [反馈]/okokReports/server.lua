	...
	if staffs[source] then
		if canAssist then
			...
			if GetPlayerPing(id) > 0 then
				...
				--TriggerClientEvent('Ultra-Notify:Alert', source, "REPORT", "您正在协助 反馈 #"..feedbackId.."!", 20000, 'info')
				TriggerClientEvent('QBCore:Notify', source, "您正在协助 反馈 #", 'info')
				TriggerClientEvent('QBCore:Notify', id, "管理员来了", 'info')
				--TriggerClientEvent('Ultra-Notify:Alert', id, "REPORT", "管理员来了!", 20000, 'info')

				if Webhook ~= '' then
					assistFeedbackWebhook(assistFeedback)
				end
			else	
				--TriggerClientEvent('Ultra-Notify:Alert', id, "REPORT", "该玩家已不在服务器中!", 20000, 'error')
				TriggerClientEvent('QBCore:Notify', id, "该玩家已不在服务器中", 'error')
			end
