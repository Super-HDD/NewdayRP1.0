				...
				if IsControlJustReleased(0, QBCore.Shared.Keybinds['J']) then
					QBCore.Functions.TriggerCallback('police:GetCops', function(result)
						...
						if CurrentLawmen >= Config.MinimumLawmen then
							...
							if hasItem then
								TriggerServerEvent('qb-rhodesbankheist:server:removeItem', 'lockpick', 1)
	...
	while true do
		...
		if object ~= 0 and robberystarted == true and dynamiteused == false then
			...
			if #(pos - objectPos) < 3.0 then
				...
				if IsControlJustReleased(0, QBCore.Shared.Keybinds['J']) then
					TriggerEvent('qb-rhodesbankheist:client:boom')
				end
			end
		end
		if awayFromObject then
...
-- blow vault doors
RegisterNetEvent('qb-rhodesbankheist:client:boom')
AddEventHandler('qb-rhodesbankheist:client:boom', function()
	if robberystarted == true then
		local hasItem = QBCore.Functions.HasItem('dynamite', 1)
		if hasItem then
			dynamiteused = true
			TriggerServerEvent('qb-rhodesbankheist:server:removeItem', 'dynamite', 1)
			...
			TriggerEvent('qb-rhodesbankheist:client:policenpc')
			local alertcoords = GetEntityCoords(PlayerPedId())
			TriggerServerEvent('police:server:policeAlert', '罗德银行遭抢劫')
		else
...
Citizen.CreateThread(function()
	exports['qb-core']:createPrompt('vault1', vector3(1288.6381, -1313.991, 77.039779), 0xF3830D8E, '战利品库', {
		type = 'client',
		event = 'qb-rhodesbankheist:client:checkvault1',
		args = {},
	})
end)

-- loot vault1
RegisterNetEvent('qb-rhodesbankheist:client:checkvault1', function()
	...
	if robberystarted == true and vault1 == false then
			...
			SetCurrentPedWeapon(player, `WEAPON_UNARMED`, true)
			TriggerServerEvent('qb-rhodesbankheist:server:reward')
			vault1 = true
	else
...
Citizen.CreateThread(function()
	exports['qb-core']:createPrompt('vault2', vector3(1286.2711, -1315.305, 77.039764), 0xF3830D8E, '战利品库', {
		type = 'client',
		event = 'qb-rhodesbankheist:client:checkvault2',
		args = {},
	})
end)

-- loot vault2
RegisterNetEvent('qb-rhodesbankheist:client:checkvault2', function()
	local player = PlayerPedId()
	SetCurrentPedWeapon(player, `WEAPON_UNARMED`, true)
...
-- start mission npcs
RegisterNetEvent('qb-rhodesbankheist:client:policenpc')
AddEventHandler('qb-rhodesbankheist:client:policenpc', function()
...
Citizen.CreateThread(function()
	exports['qb-core']:createPrompt('rhopolicelock', vector3(1295.91, -1304.38, 77.04), QBCore.Shared.Keybinds['J'], '紧急菜单', {
		type = 'client',
		event = 'qb-rhodesbankheist:client:bankmenu',
		args = {},
	})
end)

-- emergency menu
RegisterNetEvent('qb-rhodesbankheist:client:bankmenu', function()
    exports['qb-menu']:openMenu({
        ...
        },
        {
			...
			icon = "fas fa-lock",
            params = {
                event = 'qb-rhodesbankheist:client:policelock',
				isServer = false,
            }
        },
        {
            ...
            params = {
                event = 'qb-rhodesbankheist:client:policeunlock',
				isServer = false,
            }
        },
...
RegisterNetEvent('qb-rhodesbankheist:client:policelock', function()
...
RegisterNetEvent('qb-rhodesbankheist:client:policeunlock', function()
    QBCore.Functions.GetPlayerData(function(PlayerData)
        if PlayerData.job.name == "police" then
			-- lock doors
			for k,v in pairs(Config.VaultDoors) do
