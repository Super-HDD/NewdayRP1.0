...
-- give reward
RegisterServerEvent('qb-rhodesbankheist:server:reward')
AddEventHandler('qb-rhodesbankheist:server:reward', function()
    local src = source
	...
	elseif chance > 80 then -- large reward
		...
		Wait(5000)
		TriggerClientEvent('QBCore:Notify', src, '额外的'..Config.MoneyRewardAmount..''..Config.MoneyRewardType..'被抢劫了！', 'primary')
	end
end)

-- remove item
RegisterNetEvent('qb-rhodesbankheist:server:removeItem', function(item, amount)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
