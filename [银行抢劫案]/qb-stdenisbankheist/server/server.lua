...
-- give reward
RegisterServerEvent('qb-stdenisbankheist:server:reward')
AddEventHandler('qb-stdenisbankheist:server:reward', function()
    local src = source
	...
	elseif chance > 80 then -- large reward
		...
		Wait(5000)
		TriggerClientEvent('QBCore:Notify', src, '额外的'..Config.MoneyRewardAmount..''..Config.MoneyRewardType..'被抢劫！', 'primary')
	end
end)

-- remove item
RegisterNetEvent('qb-stdenisbankheist:server:removeItem', function(item, amount)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
