...
-- give reward
RegisterServerEvent('qb-bwbankheist:server:reward')
AddEventHandler('qb-bwbankheist:server:reward', function()
    local src = source
...
-- remove item
RegisterNetEvent('qb-bwbankheist:server:removeItem', function(item, amount)
	local src = source
    local Player = QBCore.Functions.GetPlayer(src)
