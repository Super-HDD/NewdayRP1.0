...
QBCore.Functions.CreateCallback('admin:server:getrank', function(source, cb)
...
RegisterNetEvent('admin:server:kill', function(player)
    ...
    if QBCore.Functions.HasPermission(src, permissions['kill']) or IsPlayerAceAllowed(src, 'command')  then
        TriggerClientEvent('qb-medic:client:KillPlayer', player.id)
    else
        BanPlayer(src)
    end
end)
...
RegisterNetEvent('admin:server:revive', function(player)
    ...
    if QBCore.Functions.HasPermission(src, permissions['revive']) or IsPlayerAceAllowed(src, 'command')  then
        ...
        TriggerClientEvent('qb-medic:clent:playerRevive', src)
        --RevivePlayer()
    else
        BanPlayer(src)
    end
...
RegisterNetEvent('admin:server:cloth', function(targetId)
    ...
    if QBCore.Functions.HasPermission(src, permissions['cloth']) or IsPlayerAceAllowed(src, 'command') and targetPlayer  then
...
RegisterNetEvent('admin:server:cloth', function(player)
    local src = source
    if QBCore.Functions.HasPermission(src, permissions['cloth']) or IsPlayerAceAllowed(src, 'command') then
        TriggerEvent("qb-clothes:LoadClothes", src)
    else
        BanPlayer(src)
    end
end)--]]
...
RegisterServerEvent("admin:server:SetWeather", function(weatherType)
    local src = source
    if QBCore.Functions.HasPermission(src, 'weather') or IsPlayerAceAllowed(src, 'command') then
...
RegisterServerEvent("admin:server:DeleteCharacter", function(citizenId,reason)
    local src = source
    if QBCore.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
        local playerId = citizenId
...
QBCore.Functions.CreateCallback("slrp-admin:server:HasPermission", function(source, cb, permission)
...
QBCore.Commands.Add('admin', Lang:t("commands.open_admin"), {}, false, function(source, _)
    if QBCore.Functions.HasPermission(source, 'admin') then
        ...
        QBCore.Functions.Kick(source, "没有权限", nil, nil)
    end
end, 'admin')
