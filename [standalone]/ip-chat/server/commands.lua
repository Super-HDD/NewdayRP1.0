    ...
    RegisterCommand(Config.AdvertisementCommand, function(source, args, rawCommand)
        ...
        if canAdvertise then
            if bankMoney >= Config.AdvertisementPrice then
                ...
                QBCore.Functions.Notify("Advertisement successfully made for "..Config.AdvertisementPrice..'$', success, 5000)
                ...
                QBCore.Functions.Notify("You don't have enough money to make an advertisement", 'error')
            end
        else
            QBCore.Functions.Notify("You can only advertise once every "..Config.AdvertisementCooldown.." minutes.", 'error', 5000)
        end
    end)
end
...
RegisterCommand('reply', function(source, args, rawCommand)
    ...
    if QBCore.Functions.HasPermission(src, 'admin') or IsPlayerAceAllowed(src, 'command') then
        ...
        local reportedPlayer = QBCore.Functions.GetPlayer(report.reporter)
