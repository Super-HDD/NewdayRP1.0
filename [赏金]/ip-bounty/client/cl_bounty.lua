...
CreateThread(function()
	for _, v in pairs(Config.LocationsB) do
		exports['qb-target']:AddCircleZone(v.location, v.coords, 1, {
			name = v.name,
			debugPoly = false,
		}, {
			options = {
			...
			if starting then
				if timer <= 0 then
					if Config.DefaultNotification then
						QBCore.Functions.Notify('Time is up!, You have failed to complete the mission in time.', 'inform', 4000)
					else
					...
					for k,v in pairs(createdped) do
						if IsEntityDead(v) then
							...
							if distance < 25.0 then
								if Config.DefaultNotification then
									QBCore.Functions.Notify('Kill All The Targets', 'inform', 4000)
								else
					...
					if IsPlayerDead(pl) then
						...
						if allTargetsDead == true then
							...
							if Config.DefaultNotification then
								QBCore.Functions.Notify('success knock out all the opponents', 'success', 4000)
							else
...
function GameTimer()
    timer = 2400 -- set timer to 20 minutes (20 mins * 60 secs/min = 1200 seconds)
    local sleep = 1000
    CreateThread(function()
        while starting do
            Wait(sleep)
...
function StartDialog()
	Citizen.CreateThread(function()
		...
		while timetocheck >= 0 do
			Wait(0)
			if Config.DefaultNotification then
				QBCore.Functions.Notify(Config.KillingMessage, 'inform', 5000)
			else
...
RegisterNetEvent('ip-bounty:client:menu', function()
	local hasItem = QBCore.Functions.HasItem(Config.TicketBounty, 1)
	...
	elseif not hasItem then
		if Config.DefaultNotification then
			QBCore.Functions.Notify('you need a ticket to start bounty', 'inform', 5000)
		else
			TriggerEvent("ip-core:ShowTopNotification", "You Dont Have a Bounty Ticket", "To Start Bounty", 5000)
		end
	elseif oncd == true then
		if Config.DefaultNotification then
			QBCore.Functions.Notify('bounty on cooldown', 'inform', 5000)
		else
