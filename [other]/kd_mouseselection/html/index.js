...
$(function () {
    ...
    function clickOn(e, data) {
        var level = parseInt(e.srcElement.getAttribute('level'));
    ...
    function CreateItem(data)
    {
        ...
        if (!data.children) {
            node.addEventListener("click", (e) => {clickOn(e,data)})
        } else {
    ...
    function ManageScroll(parent) {
        let div = parent.firstElementChild
        var windowWidth = window.innerWidth;
        var windowHeight = window.innerHeight;

        // Obtenez les dimensions et la position de la div
        var divWidth = div.offsetWidth;
        var divHeight = div.offsetHeight;
        var divTop = div.offsetTop;
        var divLeft = div.offsetLeft;

        // Vérifiez si la div dépasse de la fenêtre du navigateur
        if (divTop < 0 || divLeft < 0 || divTop + divHeight > windowHeight || divLeft + divWidth > windowWidth) {
            div.style.overflowY = 'scroll';
            let height = $(".menu-item")[0].offsetHeight
            let newheight = (windowHeight - divTop - 20) - ((windowHeight - divTop - 20)%height)
