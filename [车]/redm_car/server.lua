...
elseif Config.framework == "qbr2" then 
  qc = "qb-core"
  QBCore = exports[qc]:GetCoreObject()
end
...
function GetPlayerData(src)
  ...
  elseif Config.framework == "qbr" or Config.framework == "qbr2" then 
      ...
      if  Config.Framework == "qbr" then
          ...
          User = QBCore.Functions.GetPlayer(src)
      end
...
RegisterServerEvent("redm_ford:open_menu", function(id)
  ...
  if player then 
    ...
    if go then 
      TriggerClientEvent("redm_ford:open_menu_c", src, id)
    else
      TriggerClientEvent("Notification:redm_car", src, Config.Texts.CarDealer, Config.Texts.NoJob, Config.Textures.locked[1], Config.Textures.locked[2], 2500)
    end
  end
end)
--------------------------------------------------------------------------------------------------------------------------------------------
  ...
  if not dat then 
    if Config.Carmodels[id] then 
      ...
      if player.money >= price then 
        ...
        RemovePMoney(src, price)
        TriggerClientEvent("Notification:redm_car", src, Config.Texts.CarDealer, Config.Texts.CarPurchased, Config.Textures.tick[1], Config.Textures.tick[2], 2500)
      else
        TriggerClientEvent("Notification:redm_car", src, Config.Texts.CarDealer, Config.Texts.NoMoney, Config.Textures.locked[1], Config.Textures.locked[2], 2500)
      end
    end
  else
    TriggerClientEvent("Notification:redm_car", src, Config.Texts.CarDealer, Config.Texts.AlreadyHave, Config.Textures.locked[1], Config.Textures.locked[2], 2500)
  end
end)
--------------------------------------------------------------------------------------------------------------------------------------------
    ...
    if id > dat[1].engine then 
      if id == (dat[1].engine+1) then 
        if Config.Upgrades[id] then 
          ...
          if player.money >= price then 
            TriggerClientEvent("redm_ford:upgrade_update", src, id)
            UpdateSQL({identifier=player.identifier, charid = player.charid}, id)
            RemovePMoney(src, price)
            TriggerClientEvent("Notification:redm_car", src, Config.Texts.CarDealer, Config.Texts.UpgradePurchased.." ("..Config.Upgrades[id].label..")", Config.Textures.tick[1], Config.Textures.tick[2], 2500)
          else
            TriggerClientEvent("Notification:redm_car", src, Config.Texts.CarDealer, Config.Texts.NoMoney, Config.Textures.locked[1], Config.Textures.locked[2], 2500)
          end
        end
      else
        TriggerClientEvent("Notification:redm_car", src, Config.Texts.CarDealer, Config.Texts.BuyPrevious.." ("..(dat[1].engine+1)..")", Config.Textures.locked[1], Config.Textures.locked[2], 2500)
      end
    else
      TriggerClientEvent("Notification:redm_car", src, Config.Texts.CarDealer, Config.Texts.BetterEngine, Config.Textures.locked[1], Config.Textures.locked[2], 2500)
    end
  else
    TriggerClientEvent("Notification:redm_car", src, Config.Texts.CarDealer, Config.Texts.NoCar, Config.Textures.locked[1], Config.Textures.locked[2], 2500)
  end
end)
--------------------------------------------------------------------------------------------------------------------------------------------
RegisterServerEvent("redm_ford:spawn_car", function(id)
  ...
  if dat then 
    ...
    TriggerClientEvent("redm_ford:spawn_c", src, spawn, Config.Carmodels[dat[1].model], dat[1].engine)
  else
    TriggerClientEvent("Notification:redm_car", src, Config.Texts.CarDealer, Config.Texts.NoCar, Config.Textures.locked[1], Config.Textures.locked[2], 2500)
  end
end)
